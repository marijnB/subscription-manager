...
export class SubscribersService {
  ...
  saveSubscriber(inputSubscriber,inputTopic){
      ...
      console.log(matchingSubscriber.length);
      ...
      if (matchingSubscriber.length << 1){
             SUBSCRIBERS.forEach((subscriber)=>{
                  if(subscriber.mail.toLowerCase()  == inputSubscriber.toLowerCase() ){
                      let matched = subscriber.topics.find(matchTopic);
                      // if no topic found : otherwise saveFlag is false ( topic already on subscriber)
